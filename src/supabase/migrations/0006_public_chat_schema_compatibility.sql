...
set status = case
  when status = 'active' then 'ready_pending_mode'
...
drop policy if exists "Owners can manage own public pages" on public.property_public_pages;
...
create index if not exists conversations_property_session_idx on public.conversations(property_id, public_session_id);
create index if not exists conversations_owner_property_idx on public.conversations(owner_id, property_id);
